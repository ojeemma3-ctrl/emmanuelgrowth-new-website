...
export function ContactTrigger({ className, children }: { className: string; children: ReactNode }) {
  ...
  return <button className={className} onClick={openContact} type="button" aria-haspopup="dialog">{children}</button>;
}

export function AvaTrigger({ className, children }: { className: string; children: ReactNode }) {
  const { openAva } = useHomeInteractions();
  return <button className={className} onClick={openAva} type="button" aria-haspopup="dialog">{children}</button>;
}
